...
같은 태그를 달면 한 화면에 모입니다. 예를 들어 week01·week02·week03 노트에
모두 `python` 태그를 달면, `python` 태그를 눌러 3주치를 한 번에 볼 수 있습니다.
...
- 노트 제목 아래에 태그가 표시되고, 누르면 같은 태그 노트가 모입니다.
- 사이드바의 **태그별 모아보기**(`docs/tags.md`)에서 전체 태그 목록을 봅니다.
- 태그는 소문자로 통일하는 등 규칙을 정하면 좋습니다 (`python` 과 `Python` 은 다른 태그).
...
| 단축어 | 나오는 것 |
|---|---|
| `til` | 파일 전체 기본 틀 (title, tags, 섹션, 코드 구간까지) |
| `sec` | `#== ` (섹션 제목) |
| `desc` | `#> ` (설명) |
| `stuck` | `#! ` (배운 점·막힌 곳) |
| `todo` | `#? ` (나중에 볼 것) |
| `issue` | `#@@@ ` (공부 주제 → 이슈) |
| `fold` | `#(1)> ` (한 줄 강조 + 접이식 설명) |
| `foldn` | `#(1:5)> ` (여러 줄 강조 + 설명, 줄 수 입력) |
| `snip` | `# --8<-- [start]` / `[end]` (코드 구간 표시) |

`til` 은 파일을 새로 시작할 때, `snip` 은 한 파일에 섹션을 더 추가할 때 씁니다.
접이식 설명이 여러 줄이면 첫 줄만 `foldn`(줄 수 지정), 나머지는 `fold` 로 이어 쓰면 됩니다.
