...
date: 2026-08-02
tags: [python, string, 1주차]
---

# 문자열 슬라이싱과 내장 함수

> 원본 코드: [`02_string.py`](02_string.py)
...
```
<div class="til-note" data-til-line="2" hidden>시작·끝 비우면 전체, 간격 2 → 인덱스 0,2,4... 만 남아 '-' 가 빠짐<br>음수는 뒤에서부터. [:-4] = 뒤 4글자 뺀 나머지</div>
</div>
...
```
<div class="til-note" data-til-line="2" hidden>괄호 없으면 c 에만 곱해져서 '파이썬썬썬'</div>
</div>
...
```
<div class="til-note" data-til-line="7" hidden>인자 비우면 공백 기준 + 반환값이 문자열이 아니라 리스트[]</div>
<div class="til-note" data-til-line="13" hidden>'#-' 는 "# 또는 -" 라는 글자 목록. 덩어리가 아님<br>lstrip() 왼쪽만 · rstrip() 오른쪽만</div>
</div>
...
```
<div class="til-note" data-til-line="3" hidden>양끝 _ 은 strip, 사이 공백은 replace</div>
<div class="til-note" data-til-line="7" hidden>두 슬라이스 모두 원본 txt 기준으로 먼저 계산된 뒤 실행</div>
<div class="til-note" data-til-line="11" hidden>새 문자열을 반환하니 점으로 이어 쓸 수 있음</div>
</div>
