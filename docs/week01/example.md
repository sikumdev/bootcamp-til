...
def create_post(post: Postcreate,db: Session = Depends(get_db)):
    ...
    return created_post
```
<div class="til-note" data-til-line="4" hidden>post.model_dump() -> Postcreate 객체(pydantic)를 딕셔너리 형태로 변환<br>{"title": "제목입니다", "author": "홍길동", "content": "내용입니다"}<br>** 은 키워드 인자로 풀어줌<br>Post(title="제목입니다", author="홍길동", content="내용입니다")<br>즉, DB 객체를 가져오는거임</div>
</div>
...
def update_post(post_id: int, post_update: PostUpdate, db: Session = Depends(get_db)):
    ...
    return post
```
<div class="til-note" data-til-line="12" hidden>사용자가 실제로 보낸(None이 아닌) 필드만 골라 수정하는 부분<br>1) model_dump()로 요청을 dict로 변환 → {"title": "새 제목", "content": None}<br>2) .items()로 (key, value) 쌍을 하나씩 꺼냄<br>3) if value is not None → 값 있는 것만 추림 → {"title": "새 제목"}<br>4) 아래 setattr(post, key, value)는 post.title = "새 제목"과 동일</div>
</div>
