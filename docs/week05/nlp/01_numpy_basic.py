...
# --8<-- [start:list_vs_array]
import numpy as np

np.set_printoptions(precision=3, suppress=True)
#(1)> precision=3 소수점 3자리까지만 표시
#(1)> suppress=True 1e-05 같은 지수 표기 대신 0.00001 로 보여줌
#(1)> 화면 표시만 바뀜. 실제 값은 그대로임
...
#(1)> 앞 리스트가 행 번호, 뒤 리스트가 열 번호. 짝을 지어 읽음
#(1)> 흩어진 위치를 골라올 때 씀. `fancy indexing` 이라고 부름
x[[1,1], [1,2]]    # [6 7]
# --8<-- [end:fancy]
...
#(3)> 그리고 각 조건을 반드시 괄호로 감쌀 것. 우선순위 때문에
...
#(2)> [[11 22 33], [14 25 36]]
#(2)> 1행: 1+10, 2+20, 3+30 / 2행: 4+10, 5+20, 6+30
#(2)> 두 행이 `같은` [10,20,30] 을 씀 → 열마다 다른 값

matrix + np.array([[100], [200]])                  # (2, 1)
#(3)> [[101 102 103], [204 205 206]]
#(3)> 1행: 전부 +100 / 2행: 전부 +200
#(3)> 행마다 `다른` 값을 씀
# --8<-- [end:broadcast_add]
...
#(1)> 1행 최댓값 3 → 0번, 2행 최댓값 4 → 3번, 3행 최댓값 5 → 2번
