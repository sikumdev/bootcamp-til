...
def parse(src, skip=0):
    ...
    note_order = []   # 번호 등장 순서
    note_line = {}    # {번호: 그 설명이 달릴 코드 줄(1-based, 강조 범위의 마지막 줄)}
    ...
    def flush():
        nonlocal code, hl, notes, note_order, note_line
    ...
    for lineno, raw in enumerate(lines, 1):
        ...
        if m and not (lineno == 1 and raw.startswith("#!/")):
            ...
            if tag == "==":
                ...
                if body and body[-1][0] == "prose":
                    # 여러 줄 #> 는 줄바꿈(<br>)을 살려 이어붙인다.
                    body[-1] = ("prose", body[-1][1] + "<br>" + text, None)
                else:
                ...
                if code:
                    ...
                    for i in range(len(code) - 1, -1, -1):
                        if code[i].strip():
                            ...
                            break
                    if last is not None:
                        note_line[num] = last + 1   # 설명은 이 줄 밑에 달림
        ...
        if inl and inl.group(1).strip():
            ...
            if num not in notes:
                notes[num] = []
                note_order.append(num)
                note_line[num] = cur   # 설명은 이 줄 밑에 달림
            if note_text:
...
def render(path, meta, body, stuck, questions):
    ...
    for kind, a, b in body:
        if kind == "head":
            ...
            hl, notes = b
            # 클릭 기능이 붙을 코드 블록임을 표시하는 컨테이너
            out.append('<div class="til-code" markdown>')
            fence = "```python"
            if hl:
            ...
            out.extend(a)
            out.append("```")
            # 설명은 숨겨두고, JS가 해당 코드 줄 바로 밑에 끼워넣는다.
            # data-til-line = 설명이 달릴 코드 줄 번호(1-based)
            for num, note_text, line_no in notes:
                parts = [p.strip() for p in note_text.split("<br>") if p.strip()]
                if not parts:
                    continue
                body_html = "<br>".join(parts)
                ln = line_no if line_no else ""
                out.append(
                    f'<div class="til-note" data-til-line="{ln}" hidden>{body_html}</div>'
                )
            out.append('</div>')
            out.append("")
